...
class MixedConvNN(object):
  ...
  """
    ...
    def __init__(self,
        ...
        self.SGD_LR = theano.shared(np.float32(0.09)
                                    )  # those 3 values are to be overwritten
        ...
        if recurrent:
            ...
            if input_depth is not None:
                self.input_shape = (batch_size, input_depth, input_size[0]
                                    )  # [batch, time, feat]
            else:
                self.input_shape = (
                    batch_size, input_size[0]
                )  # the input is repeated (see "iterations" in rnn layer)
        else:
            ...
            self._x = T.TensorType('float32',
                                   (False, ) * x_dim,
                                   name='x_cnn_input')()
            if input_dim == 3:  # strange order for theano 3dconv
                self.input_shape = (batch_size, input_size[0], input_depth,
    ...
    """
        ...
        if len(layer_input_shape) > 2:  # input_dimension >= 2
            ...
            raise ValueError(
                'Used invalid input dimension for Perceptron layer')

        input_noise = theano.shared(np.float32(
            0.2)) if enable_input_noise else None
        ...
        if self.batch_size is not None:
            num_multiplications = np.product(n_outputs) * np.product(
                layer_input_shape)
        else:
            num_multiplications = np.product(n_outputs) * np.product(
                layer_input_shape[1:])
    ...
    """
        ...
        if (layer_input_shape is None) and (layer_input is None):
            layer_input_shape = self.input_shape if len(
                self.layers) == 0 else self.layers[-1].output_shape
            layer_input = self._layer0_input if len(
                self.layers) == 0 else self.layers[-1].output
        else:
        ...
        if n_dim == 2:
            filter_shape = (nof_filters, layer_input_shape[1], filter_size[0],
                            filter_size[1])
            CL = ConvLayer2d
            if reshape:
                self._y = T.TensorType('int16',
                                       [False, False, False],
                                       name='y_cnn_labels')()
        ...
        if n_dim == 3:
            ...
            (layer_input_shape[3]+1-filter_size[1]) * (layer_input_shape[4]+1-filter_size[2]))
        if self.batch_size is not None:
        ...
        _printOps(num_multiplications)
        print "Param count:", layer.params[0].get_value(
        ).size, '+', layer.params[1].get_value().size, '=', layer.params[
            0].get_value().size + layer.params[1].get_value().size
        print '---'
        self.TotalForwardPassCost += num_multiplications
    ...
    """
        ...
        if len(layer_input_shape) == 2:
            ...
            bs = layer_input_shape[0] if (
                layer_input_shape[0] is not None) else 1
            broadcaster = (bs, iterations, layer_input_shape[1])
            layer_input = layer_input.dimshuffle(
                0, 'x', 1) * T.ones(broadcaster,
                                    dtype=theano.config.floatX)
            layer_input_shape = (layer_input_shape[0], iterations,
                                 layer_input_shape[1])
        elif len(layer_input_shape) != 3:
            raise ValueError(
                'Used invalid input dimension for Recurrent layer')
        ...
        if self.batch_size is not None:
            num_multiplications = np.product(nin) * n_hid + nin[0] * nin[
                1] * n_hid**2
        else:
            num_multiplications = np.product(nin[1:]) * n_hid + nin[
                1] * n_hid**2
        _printOps(num_multiplications)
        print '---'
        self.TotalForwardPassCost += num_multiplications
    ...
    """
        ...
        assert 0 < n_layers <= len(
            self.layers), "Number of Autoencoder layers not possible"

        chain = [self.layers[n_layers - 1]
                 ]  # if n_layers = depth(NN), add last layer, if n_layers is
        # smaller add <n_layers>th layer (s.t. a MLP remains after the AE bzw. next to it))

        first = True
        for i in xrange(n_layers - 1, -1, -
                        1):  # Invert layers starting from the deepest layer
            n_outputs = self.layers[
                i].n_in  # Get n_out and Weights from mirror layer
            W = self.layers[i].W.T if tie_W else None
            n_inputs = chain[-1].output_shape[
                1
            ]  # Get input from previous layer in chain
        ...
        self._autoencoder_chains.extend(chain[1:]
                                        )  # only keep the newly added Layers
    ...
    """
        ...
        for lay in self.layers:
            ...
            try:  # This is the output before pooling etc.
                ...
                pass
        if gradients:
            self.debug_gradients_function = opt.Optimizer(
                self).compileGradients()
    ...
    """
        ...
        self.param_count = np.sum([np.prod(p.get_value().shape)
                                   for p in self.params])

        print "Total Count of trainable Parameters:", self.param_count
        print "Building Computational Graph took %.3f s" % (
            self.t_graph - self.t_init)
        ...
        if (len(layer_last.output_shape) ==
                2) or self.n_dim != 3:  #Perceptron layer or any other
        ...
        if target == 'regression':
            ...
            self._y = T.TensorType('float32',
                                   (False, ) * n_dim_regression,
                                   name='y_cnn_regression_targets')()
            self._loss, self._loss_instance = layer_last.squared_distance(
                self._y)
            ...
            if use_class_weights:
                class_weights = T.TensorType('float32',
                                             [False],
                                             name='class_weights')()
            ...
            self._loss, self._loss_instance = layer_last.nll_mutiple_binary(
                self._y, class_weights)
            ...
            self._loss, self._loss_instance = layer_last.NLL_weak(
                self._y, class_weights)

        elif target == 'affinity':
            self._y = T.TensorType('int16',
                                   (False, ) * 5,
                                   name='y_cnn_affinity_targets')()
            if use_class_weights:
                class_weights = T.TensorType('float32',
                                             [False],
                                             name='class_weights')()
            ...
            self._loss, self._loss_instance = layer_last.NLL_affinity(
                self._y, class_weights)
            ...
            self._y = T.TensorType('int16',
                                   (False, ) * 5,
                                   name='y_cnn_affinity_targets')()
            self._y_aux.append(T.TensorType('int16',
                                            (False, ) * 4,
                                            name='y_cnn_seg_gt')())
            if use_class_weights:
                class_weights = T.TensorType('float32',
            ...
            self.malis_stats = theano.function(
                [self._x, self._y, self._y_aux[0]], ret)

        elif target == 'nll':
            if use_lazy_labels:
                if not (isinstance(layer_last, ConvLayer2d) or isinstance(
                        layer_last, ConvLayer3d)):
                    raise ValueError(
                        "Cannot use lazy labels for Percptron layer")

                mask1 = T.TensorType('int16',
                                     [False, False],
                                     name='mask_class_labeled')()
                self._y_aux.append(mask1)
                mask2 = T.TensorType('int16',
                                     [False, False],
                                     name='mask_class_not_present')()
            ...
            if use_example_weights:
                example_weights = T.TensorType('float32',
                                               (False, ) * (self._x.ndim - 1),
        ...
        if target != 'regression':
            ret = [self._loss, layer_last.errors(self._y),
                   layer_last.class_prediction]
            if target == 'nll_mutiple_binary':
                ret = [self._loss, layer_last.errors_no_tn(self._y),
                       layer_last.class_prediction]

            self.get_error = theano.function(
                [self._x, self._y] + self._y_aux, ret)
            self.class_probabilities = theano.function(
                [self._x], layer_last.class_probabilities)

        # create a list of symbolic gradients for all model parameters
        if not only_forward:
            self._gradients = T.grad(self._loss,
                                     self.params,
        ...
        if isinstance(layer_last, (ConvLayer2d, ConvLayer3d, AffinityLayer3d)):
            ...
            if self.mfp_strides is not None:
                self.output_strides = np.divide(self.output_strides,
                                                self.mfp_strides)
    ...
    **Signature**: cnn.trainingStep(data, label(, *aux)(,**kwargs))
    ...
    """
        ...
        if len(args) != (len(self._y_aux) + 2):
            raise ValueError(
                "The number of auxiliary arguments for the NLL is not matching the compiled signature:\
      %s. Got %i auxiliary args." % (self._y_aux, len(args) - 2))
        ...
        if mode == 'SGD':
            ...
            loss, loss_instance = self.CG(*args
                                          )  # this already is updated loss
            ...
            loss, loss_instance = self.Adam(*args
                                            )  # this already is updated loss
    ...
    """
        if weight_decay == False:
            ...
            self.global_weightdecay.set_value(
    ...
    def setDropoutRates(self, rates):
        ...
        for lay, ra, i in zip(self.layers, rates, range(len(rates))):
            try:
                assert 1.0 >= np.float32(
                    ra) >= 0, "Dropout rates must be [0,1]"
    ...
    def _predictDenseTile(self, raw_img, out_arr, offset):
      ...
      """
        ...
        if np.all(np.equal(self.output_strides, 1)):
            if self.n_dim == 2:
                out_arr[:, 0] = self.class_probabilities(raw_img[None])[
                    0
                ]  # (ch,x,y)
            else:
                out_arr[:] = self.class_probabilities(raw_img[None])[
                    0
                ]  # (z,ch,x,y)
            ...
            for x_off in range(self.output_strides[-2]):
                for y_off in range(self.output_strides[-1]):
                    if self.n_dim == 2:
                        cut_img = raw_img[
                            None, :, x_off:x_off + self.patch_size[
                                0], y_off:y_off + self.patch_size[1]]
                        ...
                        out_arr[:, 0, x_off::self.output_strides[0], y_off::
                                self.output_strides[
                                    1]] = self.class_probabilities(cut_img)[0]

                    elif self.n_dim == 3:
                        for z_off in range(self.output_strides[0]):
                            cut_img = raw_img[
                                None, z_off:z_off + self.patch_size[
                                    0], :, x_off:x_off + self.patch_size[
                                        1], y_off:y_off + self.patch_size[2]]

                            #prob = self.class_probabilities(cut_img)[0]
                            out_arr[:, z_off::self.output_strides[
                                0], x_off::self.output_strides[1], y_off::
                                    self.output_strides[
                                        2]] = self.class_probabilities(
                                            cut_img)[0]
    ...
    """
        ...
        if self.dimension_calc is not None:
            ...
            assert offset is not None, "If the cnn has not been intialised with a dimension calculator object,\
      you must pass the offset to this function explicitly"

            offset = np.floor(offset).astype(np.int)

        n_lab = self.n_lab
        cnn_out_sh = self.output_shape[
            2:
        ]  # without batch size and channel/n_lab
        ps = self.patch_size
        ...
        if pad_raw:
            raw_img = np.pad(raw_img,
                             [(0, 0), (offset[1], offset[1]),
                              (offset[2], offset[2]), (offset[0], offset[0])],
                             mode='symmetric')
        ...
        prob_arr = np.zeros(
            np.concatenate([[self.n_lab, ], prob_sh]),
            dtype=np.float32)  # zxy

        pred_sh = np.array([raw_sh[0] - 2 * offset[1], raw_sh[1] - 2 *
                            offset[2], raw_sh[2] - 2 * offset[0]])  # xyz
        if as_uint8:
            predictions = np.zeros(
                np.concatenate(([n_lab, ], pred_sh)),
                dtype=np.uint8)  # xyz
        else:
            predictions = np.zeros(
                np.concatenate(([n_lab, ], pred_sh)),
                dtype=np.float32)  # xyz

        if self._atleast_single_mfp and not np.all(np.equal(
                self.output_strides, 1)):
            raise NotImplementedError(
                "If MFP is partially enabled, the dense prediction does not work atm")
        ...
        print "Predicting img", raw_img.shape, "in", total_nb_tiles, "Blocks:", (
        ...
        for x_t in range(x_tiles):
            for y_t in range(y_tiles):
                for z_t in range(z_tiles):
                    ...
                    raw_tile = raw_img[:, x_t * prob_sh[1]:x_t * prob_sh[
                        1] + tile_sh[1], y_t * prob_sh[2]:y_t * prob_sh[
                            2] + tile_sh[2], z_t * prob_sh[0]:z_t * prob_sh[0]
                                       + tile_sh[0]]

                    this_is_end_tile = False if np.all(np.equal(raw_tile.shape[
                        1:], np.roll(tile_sh, 2))) else True

                    if this_is_end_tile:  # requires 0-padding
                        right_pad = np.subtract(
                            np.roll(tile_sh,
                                    2), raw_tile.shape[1:])  # (ch,x,y,z)
                        right_pad = np.concatenate(
                            ([0, ], right_pad))  # for channel dimension
                    ...
                    if self.n_dim == 2:
                        ...
                        prob_arr = self._predictDenseTile(
                            raw_tile[..., 0], prob_arr, offset
                        )  # returns (ch,z=1,x,y)
                        prob = prob_arr[:, 0, :, :, None
                                        ]  # (ch,z=1,x,y) -> (ch,x,y,z=1)
                    else:
                        raw_tile = np.transpose(raw_tile, (3, 0, 1, 2)
                                                )  # (ch,x,y,z) -> (z,ch,x,y)
                        prob_arr = self._predictDenseTile(raw_tile, prob_arr,
                                                          offset)
                        prob = np.transpose(prob_arr, (0, 2, 3, 1)
                                            )  # (ch,z,x,y) -> (ch,x,y,z)

                    if this_is_end_tile:  # cut away padded range
                        prob = prob[:, :prob_sh[1] - right_pad[1], :prob_sh[2]
                                    - right_pad[2], :prob_sh[0] - right_pad[3]]

                    if as_uint8:
                        prob *= 255
                        prob = prob.astype(np.uint8)  # maybe not needed...

                    predictions[:, x_t * prob_sh[1]:(x_t + 1) * prob_sh[
                        1], y_t * prob_sh[2]:(y_t + 1) * prob_sh[2], z_t *
                                prob_sh[0]:(z_t + 1) * prob_sh[0]] = prob
                    ...
                    if show_progress:
                        ...
                        if progress <= 100:
                            ...
                            sys.stdout.write(
                                '\rProgress: %.2f%% in %s; estimate: %s' %
                                (progress, dtime, estimate))
                            sys.stdout.flush()

        sys.stdout.write(' - done\n'.decode("string_escape"))
        sys.stdout.flush()
        print "Inference speed: %.3f MB or MPix /s\n" % (
            np.product(predictions.shape[1:]) * 1.0 / 1000000 /
    ...
    def get_nonpooled_activities(self, data):
        n = len(self.layers)
        activities = []
        for layer, dbgf, i in zip(self.layers, self.debug_conv_output,
                                  range(n)):
            activities.append(dbgf(data))
        return activities
    ...
    def saveParameters(self, path='CNN.save', layers=None, show=True):
        ...
        f = open(path, 'w')
        if layers is None:
            n_lay = len(self.layers) - len(
                self._autoencoder_chains
            )  # exclude the AE chains (they have only shared W)
        ...
        for lay in layers:
            ...
            if len(lay.params) > 2:  # Recurrent Params
                cPickle.dump(lay.params[2].get_value(borrow=True),
                             f,
                             protocol=2)
                cPickle.dump(lay.params[3].get_value(borrow=True),
                             f,
                             protocol=2)

        cPickle.dump(self.poolings,
                     f,
                     protocol=2)  # list of all pooling factors
        f.close()
    ...
    """
        ...
        if strict:
            ...
            self._loadParametersAdaptive(myfile,
                                         n_layers_to_load=n_layers_to_load)
    ...
    """
        ...
        print "Shapes of current Net are:", [lay.params[0].get_value(
            borrow=True).shape for lay in self.layers]
        ...
        for it, layer in enumerate(self.layers):
            ...
            if save_shape == target_shape:
                ...
                if (target_shape[0] >
                        save_shape[0]):  #need more filters than in save
                    for i in range(0, target_shape[0], save_shape[0]):
                        if (target_shape[1] > save_shape[1]):
                            for j in range(0, target_shape[1], save_shape[1]):
                                temp[
                                    i:min(target_shape[0], save_shape[0] + i),
                                    j:min(target_shape[1], save_shape[1] +
                                          j), :min(target_shape[2], save_shape[
                                              2]), :
                                    min(target_shape[3], save_shape[3])] = p[:(
                                        min(target_shape[0], save_shape[0] + i)
                                        - i), :min(target_shape[
                                            1] - j, save_shape[1]), :min(
                                                target_shape[2], save_shape[
                                                    2]), :min(target_shape[3],
                                                              save_shape[3])]
                        else:
                            temp[i:min(target_shape[0], save_shape[0] +
                                       i), :target_shape[1], :min(target_shape[
                                           2], save_shape[2]), :
                                 min(target_shape[3], save_shape[3])] = p[:(
                                     min(target_shape[0], save_shape[0] + i) -
                                     i), :target_shape[1], :min(target_shape[
                                         2], save_shape[2]), :min(target_shape[
                                             3], save_shape[3])]

                else:
                    if (target_shape[1] > save_shape[1]):
                        for j in range(0, target_shape[1], save_shape[1]):
                            temp[:min(target_shape[0], save_shape[0]), j:min(
                                target_shape[1], save_shape[1] + j), :min(
                                    target_shape[2], save_shape[2]), :
                                 min(target_shape[3], save_shape[3])] = p[:min(
                                     target_shape[0], save_shape[0]), :min(
                                         target_shape[1] - j,
                                         save_shape[1]), :min(target_shape[
                                             2], save_shape[2]), :min(
                                                 target_shape[3], save_shape[
                                                     3])] + np.random.rand(
                                                         min(target_shape[0],
                                                             save_shape[0]),
                                                         min(target_shape[1],
                        ...
                        if target_shape[1] < save_shape[1]:
                            mid_offset = int(
                                (save_shape[1] - target_shape[1]) / 2.)

                        temp[:min(target_shape[0], save_shape[
                            0]), :target_shape[1], :min(target_shape[2],
                                                        save_shape[2]), :
                             min(target_shape[3], save_shape[3])] = p[:min(
                                 target_shape[0], save_shape[
                                     0]), mid_offset:mid_offset + target_shape[
                                         1], :min(target_shape[2], save_shape[
                                             2]), :min(target_shape[3],
                                                       save_shape[3])]
                ...
                temp = np.float32(np.random.normal(
                    0, np.std(p) + 1e-9, target_shape))  #/6.*n_params_ratio
                ...
                temp[nf_start:nf_end, f_st:f_end, c_st:c_end, f_st:f_end, f_st:
                     f_end] = p[nf_start:nf_end, f_st_:f_end_, c_st_:c_end_,
                                f_st_:f_end_, f_st_:f_end_]

                layer.params[0].set_value(temp, borrow=False)

            else:
                print "Load: skipping layer" + str(it + 1) + (
                    "" if len(target_shape) == 4 else "-\
            ...
            if target_shape[0] == save_shape[0]:
                ...
                layer.params[1].set_value(
                    p[:min(target_shape[0], save_shape[0])],
                    borrow=False)
            else:
                temp = np.float32(np.random.uniform(
                    1e-5 + (0.5 if layer.activation_func in ["sigmoid", "relu"]
                            else 0), 1e-6, target_shape))
                for i in range(0, target_shape[0], save_shape[0]):
                    temp[i:min(target_shape[0], save_shape[0] + i)] = p[:min(
                        target_shape[0] - i, save_shape[0])]
